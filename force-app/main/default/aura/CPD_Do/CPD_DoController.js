({
    doInit : function(component, event, helper) {
        ...
        var action9 = component.get("c.getPlans");
        ...
        var action6 = component.get("c.getPlannedActivities");
        action6.setParams({recordId:record_id});
        action6.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.plannedActivities", response.getReturnValue());
            }
        });
        $A.enqueueAction(action6);
        
        var action7 = component.get("c.getInProgressActivities");
        action7.setParams({recordId:record_id});
        action7.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.inProgressActivities", response.getReturnValue());
            }
        });
        $A.enqueueAction(action7);
        
        var action8 = component.get("c.getNotStartedActivities");
        action8.setParams({recordId:record_id});
        action8.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.notStartedActivities", response.getReturnValue());
    ...
    onChangePlans : function(component, event, helper) {
        ...
        window.location.replace("/s/my-cpd-do?id=" + id);
    },
    ...
    submitIncludeStatus:function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                url = url.replace(/([&\?]tab=report*$|tab=report&|[?&]tab=report(?=#))/, '');
                if (url.indexOf('?') != -1) {
                    window.location = url+'&tab=report';
                } else {
                    window.location = url+'?tab=report';
                }
            }
